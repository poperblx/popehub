...
local dungeonLayout = {
    ...
    {
        ["floorName"] = "3",
        ["rooms"] = {"6","18","36","35","17","16","32","55","56"}
    }
}
...
function clearRoom(floorInstance, room)
    floorInstance:WaitForChild(room);
    ...
    if roomInstance:FindFirstChild("EnemySpawners") then
        ...
        if next(dungeonLayout, floorNumber) and roomInstance:FindFirstChild("ZoneCompleteModels") then
            wait(2)
...
function startAutoDungeon()
    ...
    if player.World.Value ~= "Dungeons" then
        local args = {
            [1] = "Dungeons",
            [3] = 1
        }
    ...
    for floorNumber, floor in ipairs(currentDungeonLayout) do
        ...
        for index, room in ipairs(floor.rooms) do
            ...
            repeat
                clearRoom(floorInstance, room)
            until workspace.Worlds[player.World.Value]:FindFirstChild(floor.floorName).ZonesCompleted:FindFirstChild(room) or not floorInstance:FindFirstChild(room):FindFirstChild("EnemySpawners")
            wait(5)
        end
        wait(5)
    end
